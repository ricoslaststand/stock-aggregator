...
async function retrieveStockSymbols(): Promise<Stock[]> {
    const results = await axios.get('https://pkgstore.datahub.io/core/s-and-p-500-companies/constituents_json/data/297344d8dc0a9d86b8d107449c851cc8/constituents_json.json')
    ...
    try {
        ...
        await PromisePool
            ...
            .onTaskFinished((stock) => {
                console.log(`Processed daily stocks for ${stock.ticker_symbol} from ${startDate} to ${endDate}`)
            })
            ...
            .process(async (stock, index) => {
                ...
                try {
                    await db('daily_stock_prices').insert(formattedResults).onConflict(['ticker_symbol', 'date']).ignore()
                } catch (err) {
                    console.log(err)
...
async function getCalendarEvents(symbol: string) {
    const results = await yahooFinance.quoteSummary(symbol, { modules: [ "calendarEvents" ] });
